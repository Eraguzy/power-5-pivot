...
int pivotBoard(int lines, int columns, char** tab) {
    ...
    do{
        ...
        displayer(lines, columns, tab); //displays the current state of the game
        do{ // choice
        printf("Apply this choice ?\ny:yes (apply)          n: no (set a new pivot)\n");
            checkscanf=scanf(" %c", &confirm);
            emptybuffer();
            if(checkscanf!=1 || (confirm != 'y' && confirm != 'n')) { //this part avoids bugs from scanf inputs
                printf("Please enter a valid entry ('y' or 'n').\n"); 
            }
        }while(checkscanf!=1 || (confirm != 'y' && confirm != 'n'));
    }while (confirm!='y');
    return pivotColumn;
}
